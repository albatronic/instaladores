...
class ZonaPrivadaController extends ControllerProject {
    ...
    public function LoginAction() {
        ...
        if ($usuario->getPrimaryKeyValue()) {
            // El usuario existe
            //print_r($usuario);
            //echo $usuario->getPassword(), " ", $this->request['login']['Password'], " ", md5($this->request['login']['Password'] . $this->getSemilla());
            if ($usuario->getPassword() == md5($this->request['login']['Password'] . $this->getSemilla())) {
                ...
                $usuario->queryUpdate(array("NLogin" => $usuario->getNLogin() + 1, "UltimoLogin" => date('Y-m-d H:i:s')), "Id='{$usuario->getPrimaryKeyValue()}'");
            } else {
                // Password incorrecta
    ...
    public function OlvidoAction() {
        ...
        if ($this->request['METHOD'] == 'POST') {
            if ($this->validaOlvido()) {
                ...
                //echo $nueva." ".$this->getSemilla(),"<br/>";
                $usuario->queryUpdate(array("Password" => md5($nueva . $this->getSemilla())), "Id='{$usuario->getId()}'");
                ...
                $plantilla = str_replace("#LOGO#", ($usuario->getIdPerfil()->getId() == 2) ? "bn-aaie.png" : "bn-aig.png", $plantilla);
